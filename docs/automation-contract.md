...
- The Git checksum is generated by the builder with Attune's framed, sorted
  path-and-content directory algorithm and excludes Git metadata.
...
- The entry ref and version match the source commit's root `pack.yaml`.

Consumers prefer Git but can fall back to the first independently checksummed
archive source. For this index the fallback is hosted by
`codeload.github.com`, which must be approved by the Attune deployment.
